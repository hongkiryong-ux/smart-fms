...
from models import Building, BaegunArtHallArchive, BaegunArtHallDaily

ROOT = Path(__file__).resolve().parent
SCHEMA_PATH = ROOT / "resources" / "baegun_art_hall_schema.json"
...
def is_baegun_art_hall_building(building: Building | None) -> bool:
    ...
    names.discard("")
    return "백운아트홀" in name or name in names
...
def _chiller_field_ids(schema: dict | None = None) -> list[str]:
    schema = schema or load_schema()
    ids: list[str] = []
    for group in (schema.get("chiller") or {}).get("groups") or []:
        for field in group.get("fields") or []:
            ids.append(field["id"])
    return ids


def _utility_supply_field_ids(schema: dict | None = None) -> list[str]:
    schema = schema or load_schema()
    ids: list[str] = []
    for group in (schema.get("utility_supply") or {}).get("groups") or []:
        for field in group.get("fields") or []:
            ids.append(field["id"])
    return ids
...
def empty_daily_payload() -> dict[str, Any]:
    ...
    chiller_fields = _chiller_field_ids(schema)
    supply = schema.get("utility_supply") or {}
    supply_fields = _utility_supply_field_ids(schema)
    ahu = schema.get("ahu") or {}
    ahu_fields = [f["id"] for f in ahu.get("fields") or []]
    fire = schema.get("fire") or {}
    hotwater = schema.get("hotwater") or {}
    ...
    return {
        ...
        "chiller": {
            "time": "",
            **{
                unit["id"]: {fid: "" for fid in chiller_fields}
                for unit in chiller.get("units") or []
            },
        },
        "utility_supply": {
            "time": "",
            **{
                loc["id"]: {fid: "" for fid in supply_fields}
                for loc in supply.get("locations") or []
            },
        },
        "ahu": {
            "time": "",
            **{
                unit["id"]: {fid: "" for fid in ahu_fields}
                for unit in ahu.get("units") or []
            },
        },
        "fire": {
            item["id"]: {"time": "", "pressure": ""}
            for item in fire.get("items") or []
        },
        "hotwater": {f["id"]: "" for f in hotwater.get("fields") or []},
        "electrical": _time_block(
            electrical.get("fields") or [], electrical.get("times") or ["t1", "t2", "t3"]
        ),
        "notes": "",
    }
...
def parse_daily_form(form) -> dict:
    ...
    for key, value in items:
        ...
        if key.startswith("mul__") and len(parts) == 2:
            ...
            data["chiller"]["time"] = raw
        elif key.startswith("ch__") and len(parts) == 3:
            unit, fid = parts[1], parts[2]
            if unit in data["chiller"] and isinstance(data["chiller"][unit], dict):
                if fid in data["chiller"][unit]:
                    data["chiller"][unit][fid] = raw
        elif key == "us__time":
            data["utility_supply"]["time"] = raw
        elif key.startswith("us__") and len(parts) == 3:
            loc, fid = parts[1], parts[2]
            if loc in data["utility_supply"] and isinstance(data["utility_supply"][loc], dict):
                if fid in data["utility_supply"][loc]:
                    data["utility_supply"][loc][fid] = raw
        elif key == "ahu__time":
            data["ahu"]["time"] = raw
        elif key.startswith("ahu__") and len(parts) == 3:
            unit, fid = parts[1], parts[2]
            if unit in data["ahu"] and isinstance(data["ahu"][unit], dict):
                if fid in data["ahu"][unit]:
                    data["ahu"][unit][fid] = raw
        elif key.startswith("fire__") and len(parts) == 3:
            item, field = parts[1], parts[2]
            if item in data["fire"] and field in data["fire"][item]:
                data["fire"][item][field] = raw
...
async def get_or_create_daily(
    ...
    row = BaegunArtHallDaily(building_id=building_id, log_date=log_date, data=data)
    session.add(row)
    await session.flush()
    return row
...
def compute_monthly_report(
    year: int,
    month: int,
    daily_rows: list[BaegunArtHallDaily],
    prev_month_last_row: BaegunArtHallDaily | None = None,
) -> dict[str, Any]:
    import calendar
...
async def fetch_yearly_report_data(
    ...
    rows = (
        await session.execute(
            select(BaegunArtHallDaily).where(
                BaegunArtHallDaily.building_id == building_id,
                BaegunArtHallDaily.log_date >= date(year, 1, 1),
                BaegunArtHallDaily.log_date <= date(year, 12, 31),
            )
        )
    ).scalars().all()
...
async def fetch_notes_list(
    ...
    rows = (
        await session.execute(
            select(BaegunArtHallDaily)
            .where(
                BaegunArtHallDaily.building_id == building_id,
                BaegunArtHallDaily.log_date >= date(year, month, 1),
                BaegunArtHallDaily.log_date <= date(
                    year, month, calendar.monthrange(year, month)[1]
                ),
            )
            .order_by(BaegunArtHallDaily.log_date.asc())
        )
    ).scalars().all()
    entries = []
    for row in rows:
...
async def get_building_for_qr(
    ...
    return building if is_baegun_art_hall_building(building) else None


def baegun_art_hall_daily_qr_url(building_code: str, request: Any | None = None) -> str:
    base = (os.environ.get("PUBLIC_BASE_URL") or "").strip().rstrip("/")
    if not base and request is not None:
        base = str(request.base_url).rstrip("/")
    return f"{base or 'http://127.0.0.1:8000'}/bahall/{(building_code or '').strip()}/daily"
...
async def _archive_daily(
    session: AsyncSession, building_id: int, log_date: date
) -> BaegunArtHallArchive | None:
    row = await get_daily_row(session, building_id, log_date)
    if not row:
        return None
    archive = (
        await session.execute(
            select(BaegunArtHallArchive).where(
                BaegunArtHallArchive.building_id == building_id,
                BaegunArtHallArchive.log_date == log_date,
            )
        )
    ).scalar_one_or_none()
    payload = json.dumps(row.data or {}, ensure_ascii=False, indent=2).encode("utf-8")
    filename = f"백운아트홀_운영일보_{log_date.isoformat()}.json"
    if archive:
        archive.original_name = filename
        archive.file_data = payload
        archive.file_size = len(payload)
    else:
        archive = BaegunArtHallArchive(
    ...
    async with engine.begin() as connection:
        if is_pg:
            await connection.execute(text("""
                CREATE TABLE IF NOT EXISTS baegun_art_hall_daily (
            ...
            """))
            await connection.execute(text("""
                CREATE TABLE IF NOT EXISTS baegun_art_hall_archives (
                    id SERIAL PRIMARY KEY,
                    building_id INTEGER NOT NULL REFERENCES buildings(id),
                    log_date DATE NOT NULL,
...
def export_daily_to_excel(data: dict, log_date: date) -> bytes:
    ...
    title = str(schema.get("title") or schema.get("building_name") or "백운아트홀")
        ...
        async with session_factory() as session:
            ...
            for building_id, building in rows:
                ...
                try:
                    ...
                    print(f"[bahall] rollover building={building_id}: {exc}", flush=True)
