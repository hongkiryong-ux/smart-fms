...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            ...
            from steelworks_hall import register_scheduler as register_swhall_scheduler

            register_swhall_scheduler(scheduler, AsyncSessionLocal, KST)
            from human_center import register_scheduler as register_hcenter_scheduler

            register_hcenter_scheduler(scheduler, AsyncSessionLocal, KST)
            ...
            from giga_town import register_scheduler as register_gtown_scheduler

            register_gtown_scheduler(scheduler, AsyncSessionLocal, KST)
            from park1538 import register_scheduler as register_p1538_scheduler
            ...
            from baegun_shopping import register_scheduler as register_baegun_shopping_scheduler
            register_baegun_shopping_scheduler(scheduler, AsyncSessionLocal, KST)
            scheduler.start()
        except Exception as e:
...
DASHBOARD_LAYOUT_META = {
    "ops": {
        "label": "운영 보드 (기존 화면)",
        "ref": "운영 대시보드 시안",
        "desc": "사업장 현황을 순위·정비의뢰 목록형으로 보여주는 기존 대시보드입니다. 대시보드 설정으로 구역 표시/순서를 바꿀 수 있습니다.",
    },
    "wide": {
        "label": "통합 운영 홈 (새 화면)",
        "ref": "Smart FMS 통합 대시보드 시안",
        "desc": "시안형 카드 배치. 시설·설비·정비의뢰·미해결·완료를 한눈에 봅니다. 기존 데이터·링크는 운영 보드와 동일합니다.",
    },
    "gallery": {
        "label": "시설 갤러리",
...
async def site_map_image_upload(
    ...
    """사업장 안내도 이미지 업로드 (PDF는 1페이지를 JPG로 변환). 4분할과 별개."""
...
@app.post("/admin/sites/{site_id}/grid-image")
async def site_grid_image_upload(
    site_id: int,
    file: UploadFile = File(...),
    user: User = Depends(require_can_edit),
    db: AsyncSession = Depends(get_db),
):
    """4분할 화면 전용 이미지 업로드 (안내도와 별개)."""
    from site_maps import MAP_UPLOAD_EXTS, save_site_grid_image_upload
    ...
    try:
        url = await save_site_grid_image_upload(db, site_id, content, suffix)
    except ValueError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
    except Exception as exc:
        detail = "PDF 변환에 실패했습니다." if suffix == ".pdf" else "이미지 저장에 실패했습니다."
...
async def inspection_logs2_page(
    ...
    try:
        from baegun_shopping import ensure_registered as ensure_baegun_shopping_registered
        from baegun_shopping import ensure_tables as ensure_baegun_shopping_tables

        await ensure_baegun_shopping_tables(engine)
        if await ensure_baegun_shopping_registered(db):
            await db.commit()
            from auth import invalidate_nav_cache
        ...
        await db.rollback()
        print(f"[bshop] ensure register skip: {e}", flush=True)
...
async def inspection_logs2_building_detail(
    ...
    if is_steelworks_hall_building(building):
        return RedirectResponse(
            f"/admin/inspection-logs2/{building_id}/steelworks-hall",
            status_code=303,
        )
    from human_center import is_human_center_building

    if is_human_center_building(building):
        return RedirectResponse(
    ...
    if is_eoulrim_gym_building(building):
        return RedirectResponse(
    ...
    if is_baegun_shopping_building(building):
        return RedirectResponse(
            f"/admin/inspection-logs2/{building_id}/baegun-shopping",
            status_code=303,
        )
    ...
    if is_giga_town_building(building):
        return RedirectResponse(
            f"/admin/inspection-logs2/{building_id}/giga-town",
            status_code=303,
        )
    from park1538 import is_park1538_building

    if is_park1538_building(building):
        return RedirectResponse(
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-art-hall/qr.png")
async def baegun_art_hall_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from baegun_art_hall import (
        baegun_art_hall_daily_qr_url,
        is_baegun_art_hall_building,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_baegun_art_hall_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(baegun_art_hall_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣.\-]+", "_", building.code.strip()) or "bahall"
    filename = f"{safe}_1일QR.png"
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-art-hall")
async def baegun_art_hall_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from baegun_art_hall import (
        compute_monthly_report,
        ensure_tables as ensure_bahall_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        baegun_art_hall_daily_qr_url,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_bahall_tables(engine)
    except Exception as exc:
        print(f"[bahall] ensure: {exc}", flush=True)
    ...
    if not is_baegun_art_hall_building(building):
        return RedirectResponse(
    ...
    return templates.TemplateResponse(
        ...
        "baegun_art_hall.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                baegun_art_hall_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/baegun-art-hall/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/baegun-art-hall/save")
async def baegun_art_hall_save(
    ...
    await db.commit()
    if request.headers.get("X-BAHALL-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/baegun-art-hall/close-day")
async def baegun_art_hall_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
async def baegun_art_hall_qr_daily(
    ...
    return templates.TemplateResponse(
        ...
        "baegun_art_hall.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/bahall/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/bahall/{code}/daily/save")
async def baegun_art_hall_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-shopping/qr.png")
async def baegun_shopping_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from baegun_shopping import (
        bshop_daily_qr_url,
        is_baegun_shopping_building,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_baegun_shopping_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(bshop_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣\-]+", "_", building.code.strip()) or "bshop"
    filename = f"{safe}_1일QR.png"
    disposition = "attachment" if download else "inline"
    return StreamingResponse(
        iter([data]),
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-shopping")
async def baegun_shopping_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from baegun_shopping import (
        compute_monthly_report,
        ensure_tables as ensure_bshop_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        bshop_daily_qr_url,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_bshop_tables(engine)
    except Exception as exc:
        print(f"[bshop] ensure: {exc}", flush=True)
        ...
        rows = (
            await db.execute(
                select(BaegunShoppingDaily).where(
                    ...
                    BaegunShoppingDaily.log_date >= start,
                    BaegunShoppingDaily.log_date <= end,
                )
            )
        ).scalars().all()
        previous = (
            await db.execute(
                select(BaegunShoppingDaily).where(
                    BaegunShoppingDaily.building_id == building_id,
                    BaegunShoppingDaily.log_date == start - timedelta(days=1),
                )
            )
        ).scalar_one_or_none()
        monthly = compute_monthly_report(year, month, list(rows), previous)
    ...
    return templates.TemplateResponse(
        ...
        "baegun_shopping.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                bshop_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/baegun-shopping/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/baegun-shopping/save")
async def baegun_shopping_save(
    ...
    building = await db.get(Building, building_id)
    if not building or not is_baegun_shopping_building(building):
        raise HTTPException(404)
    form = await request.form()
    log_date = date.fromisoformat(str(form.get("log_date")))
    row = await get_or_create_daily(db, building_id, log_date)
    ...
    await propagate_to_next_day(db, building_id, log_date, row.data)
    await db.commit()
    if request.headers.get("X-BSHOP-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/baegun-shopping/close-day")
async def baegun_shopping_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-shopping/export/daily")
async def baegun_shopping_export_daily(
    building_id: int,
    log_date: str = Query(...),
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    await db.commit()
    xbytes = export_daily_to_excel(row.data or {}, d)
    fname = quote(f"백운쇼핑_1일_{d.isoformat()}.xlsx")
    return StreamingResponse(
        BytesIO(xbytes),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename*=UTF-8''{fname}"},
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-shopping/export/monthly")
async def baegun_shopping_export_monthly(
    building_id: int,
    year: int = Query(...),
    month: int = Query(..., ge=1, le=12),
    user: User = Depends(require_login),
    ...
    from baegun_shopping import (
        compute_monthly_report,
        export_monthly_to_excel,
        is_baegun_shopping_building,
    )
    from models import BaegunShoppingDaily
    ...
    report = compute_monthly_report(year, month, list(rows), previous)
    xbytes = export_monthly_to_excel(report)
    fname = quote(f"백운쇼핑_월보_{year}-{month:02d}.xlsx")
    return StreamingResponse(
        BytesIO(xbytes),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename*=UTF-8''{fname}"},
...
@app.get("/admin/inspection-logs2/{building_id}/baegun-shopping/export/yearly")
async def baegun_shopping_export_yearly(
    building_id: int,
    year: int = Query(...),
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    report = await fetch_yearly_report_data(db, building_id, year)
    xbytes = export_yearly_to_excel(report)
    fname = quote(f"백운쇼핑_년보_{year}.xlsx")
    return StreamingResponse(
        BytesIO(xbytes),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename*=UTF-8''{fname}"},
...
@app.get("/bshop/{code}/daily")
async def baegun_shopping_qr_daily(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
    user: User | None = Depends(get_current_user),
    ...
    return templates.TemplateResponse(
        ...
        "baegun_shopping.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/bshop/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/bshop/{code}/daily/save")
async def baegun_shopping_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    if request.headers.get("X-BSHOP-Autosave") == "1":
        ...
        return JSONResponse({"ok": True, "data": row.data})
    return RedirectResponse(
        f"/bshop/{code}/daily?date={log_date.isoformat()}"
        f"&message={quote('저장되었습니다.')}",
        status_code=303,
    )
...
@app.get("/admin/inspection-logs2/{building_id}/steelworks-hall/qr.png")
async def steelworks_hall_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from steelworks_hall import (
        hall_daily_qr_url,
        is_steelworks_hall_building,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_steelworks_hall_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(hall_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣.\-]+", "_", building.code.strip()) or "swhall"
    filename = f"{safe}_1일QR.png"
    disposition = "attachment" if download else "inline"
    return StreamingResponse(
        iter([data]),
...
@app.get("/admin/inspection-logs2/{building_id}/steelworks-hall")
async def steelworks_hall_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from steelworks_hall import (
        compute_monthly_report,
        ensure_tables as ensure_swhall_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        hall_daily_qr_url,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_swhall_tables(engine)
    except Exception as exc:
        print(f"[swhall] ensure: {exc}", flush=True)
    ...
    return templates.TemplateResponse(
        ...
        "steelworks_hall.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                hall_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/steelworks-hall/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/steelworks-hall/save")
async def steelworks_hall_save(
    ...
    await propagate_to_next_day(db, building_id, log_date, row.data)
    await db.commit()
    if request.headers.get("X-SWHALL-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/steelworks-hall/close-day")
async def steelworks_hall_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
async def steelworks_hall_qr_daily(
    ...
    return templates.TemplateResponse(
        ...
        "steelworks_hall.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/swhall/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/swhall/{code}/daily/save")
async def steelworks_hall_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    if request.headers.get("X-SWHALL-Autosave") == "1":
        ...
        return JSONResponse({"ok": True, "data": row.data})
    return RedirectResponse(
        f"/swhall/{code}/daily?date={log_date.isoformat()}"
        f"&message={quote('저장되었습니다.')}",
        status_code=303,
    )


@app.get("/admin/inspection-logs2/{building_id}/human-center/qr.png")
async def human_center_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from human_center import (
        human_center_daily_qr_url,
        is_human_center_building,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_human_center_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(human_center_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣.\-]+", "_", building.code.strip()) or "hcenter"
    filename = f"{safe}_1일QR.png"
    disposition = "attachment" if download else "inline"
    return StreamingResponse(
        iter([data]),
...
@app.get("/admin/inspection-logs2/{building_id}/human-center")
async def human_center_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from human_center import (
        compute_monthly_report,
        ensure_tables as ensure_hcenter_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        human_center_daily_qr_url,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_hcenter_tables(engine)
    except Exception as exc:
        print(f"[hcenter] ensure: {exc}", flush=True)
    ...
    return templates.TemplateResponse(
        ...
        "human_center.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                human_center_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/human-center/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/human-center/save")
async def human_center_save(
    ...
    await propagate_to_next_day(db, building_id, log_date, row.data)
    await db.commit()
    if request.headers.get("X-HCENTER-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/human-center/close-day")
async def human_center_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
@app.get("/hcenter/{code}/daily")
async def human_center_qr_daily(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
    user: User | None = Depends(get_current_user),
    ...
    return templates.TemplateResponse(
        ...
        "human_center.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/hcenter/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/hcenter/{code}/daily/save")
async def human_center_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    if request.headers.get("X-HCENTER-Autosave") == "1":
        ...
        return JSONResponse({"ok": True, "data": row.data})
    return RedirectResponse(
        f"/hcenter/{code}/daily?date={log_date.isoformat()}"
        f"&message={quote('저장되었습니다.')}",
        status_code=303,
    )


@app.get("/admin/inspection-logs2/{building_id}/eoulrim-gym/qr.png")
async def eoulrim_gym_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from eoulrim_gym import (
        egym_daily_qr_url,
        is_eoulrim_gym_building,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_eoulrim_gym_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(egym_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣.\-]+", "_", building.code.strip()) or "egym"
    filename = f"{safe}_1일QR.png"
    disposition = "attachment" if download else "inline"
    return StreamingResponse(
        iter([data]),
...
@app.get("/admin/inspection-logs2/{building_id}/eoulrim-gym")
async def eoulrim_gym_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from eoulrim_gym import (
        compute_monthly_report,
        ensure_tables as ensure_egym_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        egym_daily_qr_url,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_egym_tables(engine)
    except Exception as exc:
        print(f"[egym] ensure: {exc}", flush=True)
    ...
    return templates.TemplateResponse(
        ...
        "eoulrim_gym.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                egym_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/eoulrim-gym/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/eoulrim-gym/save")
async def eoulrim_gym_save(
    ...
    await propagate_to_next_day(db, building_id, log_date, row.data)
    await db.commit()
    if request.headers.get("X-EGYM-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/eoulrim-gym/close-day")
async def eoulrim_gym_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
async def eoulrim_gym_qr_daily(
    ...
    return templates.TemplateResponse(
        ...
        "eoulrim_gym.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/egym/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/egym/{code}/daily/save")
async def eoulrim_gym_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
...
@app.get("/admin/inspection-logs2/{building_id}/giga-town/qr.png")
async def giga_town_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from giga_town import (
        gtown_daily_qr_url,
        is_giga_town_building,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_giga_town_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(gtown_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣.\-]+", "_", building.code.strip()) or "gtown"
    filename = f"{safe}_1일QR.png"
    disposition = "attachment" if download else "inline"
    return StreamingResponse(
        iter([data]),
...
@app.get("/admin/inspection-logs2/{building_id}/giga-town")
async def giga_town_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from giga_town import (
        compute_monthly_report,
        ensure_tables as ensure_gtown_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        gtown_daily_qr_url,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_gtown_tables(engine)
    except Exception as exc:
        print(f"[gtown] ensure: {exc}", flush=True)
    ...
    return templates.TemplateResponse(
        ...
        "giga_town.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                gtown_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/giga-town/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/giga-town/save")
async def giga_town_save(
    ...
    await propagate_to_next_day(db, building_id, log_date, row.data)
    await db.commit()
    if request.headers.get("X-GTOWN-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/giga-town/close-day")
async def giga_town_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
async def giga_town_qr_daily(
    ...
    return templates.TemplateResponse(
        ...
        "giga_town.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/gtown/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/gtown/{code}/daily/save")
async def giga_town_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    if request.headers.get("X-GTOWN-Autosave") == "1":
        ...
        return JSONResponse({"ok": True, "data": row.data})
    return RedirectResponse(
        f"/gtown/{code}/daily?date={log_date.isoformat()}"
        f"&message={quote('저장되었습니다.')}",
        status_code=303,
    )
...
@app.get("/admin/inspection-logs2/{building_id}/park1538/qr.png")
async def park1538_qr_png(
    building_id: int,
    request: Request,
    download: int = 0,
    user: User = Depends(require_login),
    ...
    from park1538 import (
        is_park1538_building,
        p1538_daily_qr_url,
        qr_png_bytes,
    )

    building = await db.get(Building, building_id)
    if not building or not is_park1538_building(building) or not building.code:
        raise HTTPException(404)
    data = qr_png_bytes(p1538_daily_qr_url(building.code, request))
    safe = re.sub(r"[^\w가-힣.\-]+", "_", building.code.strip()) or "p1538"
    filename = f"{safe}_1일QR.png"
    disposition = "attachment" if download else "inline"
    return StreamingResponse(
        iter([data]),
...
@app.get("/admin/inspection-logs2/{building_id}/park1538")
async def park1538_page(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
    ...
    from park1538 import (
        compute_monthly_report,
        ensure_tables as ensure_p1538_tables,
        fetch_notes_list,
        fetch_yearly_report_data,
        get_or_create_daily,
        is_park1538_building,
    ...
    try:
        await _ensure_inspection_log2_tables()
        await ensure_p1538_tables(engine)
    except Exception as exc:
        print(f"[p1538] ensure: {exc}", flush=True)
    ...
    return templates.TemplateResponse(
        ...
        "park1538.html",
        {
            ...
            "qr_mode": False,
            "qr_url": (
                p1538_daily_qr_url(building.code, request) if building.code else ""
            ),
            "daily_save_url": (
                f"/admin/inspection-logs2/{building_id}/park1538/save"
            ),
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
...
@app.post("/admin/inspection-logs2/{building_id}/park1538/save")
async def park1538_save(
    ...
    await propagate_to_next_day(db, building_id, log_date, row.data)
    await db.commit()
    if request.headers.get("X-P1538-Autosave") == "1":
        from starlette.responses import JSONResponse
...
@app.post("/admin/inspection-logs2/{building_id}/park1538/close-day")
async def park1538_close_day(
    building_id: int,
    request: Request,
    user: User = Depends(require_login),
    db: AsyncSession = Depends(get_db),
...
@app.get("/p1538/{code}/daily")
async def park1538_qr_daily(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
    user: User | None = Depends(get_current_user),
    ...
    return templates.TemplateResponse(
        ...
        "park1538.html",
        {
            ...
            "qr_mode": True,
            "qr_url": "",
            "daily_save_url": f"/p1538/{building.code}/daily/save",
            "error": request.query_params.get("error"),
            "message": request.query_params.get("message"),
        },
    )
...
@app.post("/p1538/{code}/daily/save")
async def park1538_qr_save(
    code: str,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    ...
    if request.headers.get("X-P1538-Autosave") == "1":
        ...
        return JSONResponse({"ok": True, "data": row.data})
    return RedirectResponse(
        f"/p1538/{code}/daily?date={log_date.isoformat()}"
        f"&message={quote('저장되었습니다.')}",
        status_code=303,
    )
