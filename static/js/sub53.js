/** 53Sub-station 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("s53-daily-form");
  const status = document.getElementById("s53-save-status");
  if (!form) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("s53-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".s53-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function isEditableCell(el) {
    return el && el.classList.contains("s53-cell") && !el.readOnly;
  }

  meterIds().forEach(syncMonthBase);

  form.querySelectorAll(".s53-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".s53-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".s53-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("s53-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("s53-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".s53-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("s53-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("s53-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".s53-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.s53-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".s53-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
