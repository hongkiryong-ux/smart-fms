/** 휴먼센터 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("hc-daily-form");
  ...
  function calcUtility() {
    form.querySelectorAll(".hc-utility-row").forEach(function (row) {
  ...
  function applyServerData(data) {
    ...
    Object.keys(data.utility).forEach(function (id) {
      const row = form.querySelector('.hc-utility-row[data-utility="' + id + '"]');
      if (!row) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("hc-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".hc-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function isEditableCell(el) {
    return el && el.classList.contains("hc-cell") && !el.readOnly;
  }

  form.querySelectorAll(".hc-utility-row").forEach(syncMonthBase);

  form.querySelectorAll(".hc-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".hc-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".hc-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("hc-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("hc-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".hc-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("hc-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("hc-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".hc-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.hc-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".hc-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
