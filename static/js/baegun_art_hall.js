/** 백운아트홀 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("bah-daily-form");
  const status = document.getElementById("bah-save-status");
  if (!form) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("bah-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".bah-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function isEditableCell(el) {
    return el && el.classList.contains("bah-cell") && !el.readOnly;
  }

  meterIds().forEach(syncMonthBase);

  form.querySelectorAll(".bah-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".bah-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".bah-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("bah-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("bah-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".bah-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("bah-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("bah-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".bah-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.bah-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".bah-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
