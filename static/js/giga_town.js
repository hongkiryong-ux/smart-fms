/** 기가타운 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("gt-daily-form");
  const status = document.getElementById("gt-save-status");
  if (!form) return;
  ...
  function calcUtility() {
    form.querySelectorAll(".gt-utility-row").forEach(function (row) {
  ...
  function applyServerData(data) {
    ...
    Object.keys(data.utility).forEach(function (id) {
      const row = form.querySelector('.gt-utility-row[data-utility="' + id + '"]');
      if (!row) return;
    ...
    if (data.equipment_run) {
      Object.keys(data.equipment_run).forEach(function (id) {
        const row = form.querySelector('.gt-eq-row[data-eq="' + id + '"]');
        if (!row) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("gt-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".gt-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function markEqPrevManual(el) {
    if (!el.classList.contains("gt-eq-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".gt-eq-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function calcEquipment() {
    form.querySelectorAll(".gt-eq-row").forEach(function (row) {
      const id = row.dataset.eq;
      const today = numberOf(row.querySelector('[name="eq__' + id + '__today_hrs"]')?.value);
      const prev = numberOf(row.querySelector('[name="eq__' + id + '__prev_cum"]')?.value);
      const monthlyInput = row.querySelector('[name="eq__' + id + '__monthly_cum"]');
  ...
  function isEditableCell(el) {
    return el && el.classList.contains("gt-cell") && !el.readOnly;
  }

  form.querySelectorAll(".gt-utility-row").forEach(syncMonthBase);

  form.querySelectorAll(".gt-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".gt-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".gt-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("gt-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("gt-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".gt-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("gt-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("gt-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".gt-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.gt-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".gt-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
