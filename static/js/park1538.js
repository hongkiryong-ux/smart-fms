/** Park1538 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("p8-daily-form");
  const status = document.getElementById("p8-save-status");
  if (!form) return;
  ...
  function calcUtility() {
    form.querySelectorAll(".p8-utility-row").forEach(function (row) {
  ...
  function applyServerData(data) {
    ...
    Object.keys(data.utility).forEach(function (id) {
      const row = form.querySelector('.p8-utility-row[data-utility="' + id + '"]');
      if (!row) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("p8-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".p8-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function isEditableCell(el) {
    return el && el.classList.contains("p8-cell") && !el.readOnly;
  }

  form.querySelectorAll(".p8-utility-row").forEach(syncMonthBase);

  form.querySelectorAll(".p8-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".p8-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".p8-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("p8-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("p8-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".p8-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("p8-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("p8-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".p8-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.p8-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".p8-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
