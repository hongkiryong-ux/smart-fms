/** 백운대 점검일지 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("bdae-daily-form");
  const status = document.getElementById("bdae-save-status");
  if (!form) return;
  ...
  function meterIds() {
    const ids = [];
    form.querySelectorAll("[data-utility]").forEach(function (el) {
      const id = el.dataset.utility;
      if (id && ids.indexOf(id) < 0) ids.push(id);
    });
    return ids;
  }

  function syncMonthBase(id) {
    const daily = numberOf(form.querySelector('[name="u__' + id + '__daily"]')?.value);
    const monthly = numberOf(form.querySelector('[name="u__' + id + '__monthly"]')?.value);
    monthBase[id] = monthly != null ? monthly - (daily || 0) : 0;
  }

  function meterMultiplier(id) {
    const el = form.querySelector('[data-utility="' + id + '"]');
    const key = el && el.dataset.multiplierKey;
    if (key) {
      const input = form.querySelector('[name="mul__' + key + '"]');
      const fromInput = numberOf(input && input.value);
      if (fromInput != null) return fromInput;
    }
    return numberOf(el && el.dataset.multiplier) || 1;
  }

  function calcUtility() {
    meterIds().forEach(function (id) {
      const multiplier = meterMultiplier(id);
      const prev = numberOf(form.querySelector('[name="u__' + id + '__prev"]')?.value);
      const today = numberOf(form.querySelector('[name="u__' + id + '__today"]')?.value);
      const dailyInput = form.querySelector('[name="u__' + id + '__daily"]');
      const monthlyInput = form.querySelector('[name="u__' + id + '__monthly"]');
      const daily = prev != null && today != null ? (today - prev) * multiplier : null;
      if (!(id in monthBase)) syncMonthBase(id);
      const monthly = daily != null ? (monthBase[id] || 0) + daily : null;
  ...
  function applyServerData(data) {
    ...
    Object.keys(data.utility).forEach(function (id) {
      ...
      const daily = form.querySelector('[name="u__' + id + '__daily"]');
      const monthly = form.querySelector('[name="u__' + id + '__monthly"]');
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("bdae-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".bdae-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function isEditableCell(el) {
    return el && el.classList.contains("bdae-cell") && !el.readOnly;
  }

  meterIds().forEach(syncMonthBase);

  form.querySelectorAll(".bdae-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".bdae-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".bdae-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("bdae-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("bdae-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".bdae-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("bdae-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("bdae-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".bdae-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.bdae-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".bdae-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
