/** 백운쇼핑 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("bs-daily-form");
  const status = document.getElementById("bs-save-status");
  if (!form) return;
  ...
  function applyServerData(data) {
    ...
    Object.keys(data.utility).forEach(function (id) {
      const row = form.querySelector('.bs-utility-row[data-utility="' + id + '"]');
      if (!row) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("bs-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".bs-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function markEqPrevManual(el) {
    if (!el.classList.contains("bs-eq-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".bs-eq-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function calcEquipment() {
    form.querySelectorAll(".bs-eq-row").forEach(function (row) {
  ...
  function isEditableCell(el) {
    return el && el.classList.contains("bs-cell") && !el.readOnly;
  }

  form.querySelectorAll(".bs-utility-row").forEach(syncMonthBase);

  form.querySelectorAll(".bs-cell").forEach(function (el) {
  ...
  function setupDragSelect(table) {
    ...
    table.querySelectorAll("tbody tr").forEach(function (tr, rowIdx) {
      tr.querySelectorAll(".bs-cell:not([readonly])").forEach(function (input, colIdx) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("bs-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("bs-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".bs-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("bs-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("bs-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".bs-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.bs-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".bs-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
