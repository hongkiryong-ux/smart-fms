/** 어울림체육관 운영일보 — UTILITY 계산 · 드래그 선택 · 엑셀 붙여넣기 · 삭제 · 자동 저장 */
(function () {
  const form = document.getElementById("eg-daily-form");
  ...
  function calcUtility() {
    form.querySelectorAll(".eg-utility-row").forEach(function (row) {
  ...
  function applyServerData(data) {
    ...
    Object.keys(data.utility).forEach(function (id) {
      const row = form.querySelector('.eg-utility-row[data-utility="' + id + '"]');
      if (!row) return;
    ...
    if (data.equipment_run) {
      Object.keys(data.equipment_run).forEach(function (id) {
        const row = form.querySelector('.eg-eq-row[data-eq="' + id + '"]');
        if (!row) return;
  ...
  function markPrevManual(el) {
    if (!el.classList.contains("eg-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".eg-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function markEqPrevManual(el) {
    if (!el.classList.contains("eg-eq-prev")) return;
    const flag = el.parentElement && el.parentElement.querySelector(".eg-eq-prev-flag");
    const isEmpty = !String(el.value || "").trim();
    if (flag) flag.value = isEmpty ? "" : "1";
  }

  function calcEquipment() {
    form.querySelectorAll(".eg-eq-row").forEach(function (row) {
  ...
  function isEditableCell(el) {
    return el && el.classList.contains("eg-cell") && !el.readOnly;
  }

  form.querySelectorAll(".eg-utility-row").forEach(syncMonthBase);

  form.querySelectorAll(".eg-cell").forEach(function (el) {
  ...
  const notes = form.querySelector(".eg-notes");
  if (notes) {
  ...
  function setupDragSelect(table) {
    ...
    function clearSelection() {
      selected.forEach(function (el) {
        el.classList.remove("eg-cell-selected");
      });
      selected.clear();
    ...
    function applySelection(r1, c1, r2, c2) {
      ...
      for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
          ...
          if (!el) continue;
          selected.add(el);
          el.classList.add("eg-cell-selected");
        }
      }
      selector.activate();
    }
    ...
    function getPasteAnchor() {
      ...
      const focused = table.querySelector(".eg-cell:focus");
    ...
    document.addEventListener("mouseup", function () {
      if (selecting) {
        selecting = false;
        form.classList.remove("eg-drag-selecting");
      }
    });
    table.addEventListener("mousedown", function () {
      form.classList.add("eg-drag-selecting");
    });
    return selector;
  }

  const selectors = [];
  form.querySelectorAll(".eg-excel").forEach(function (table) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Delete" || e.key === "Backspace") {
      if (e.target && e.target.matches && e.target.matches("input:not(.eg-cell), textarea")) return;
      let total = 0;
  ...
  document.addEventListener("mousedown", function (e) {
    if (e.target.closest(".eg-cell")) return;
    selectors.forEach(function (sel) {
      sel.clearSelection();
    });
  });
